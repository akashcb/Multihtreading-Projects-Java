...
public class Simulation {
	...
	//Your code goes here
	private static CoffeeShopManager cm = null;
	/**
	...
	public synchronized static CoffeeShopManager getCm() {
		return cm;
	}

	public static void setCm(CoffeeShopManager cm) {
		Simulation.cm = cm;
	}
	...
	public static List<SimulationEvent> runSimulation(
		...
		cm = new CoffeeShopManager(numCustomers, numCooks, numTables, machineCapacity, randomOrders);
		...
		if (!randomOrders) {
			...
			for(int i = 0; i < customers.length; i++) {
				...
				customers[i] = new Thread(
						new Customer("Customer " + (i+1), order), "custThread-"+i
						);
			}
		}
...
//				cooks[i].join();
			...
			{
				System.out.println("Stopping cooks");
				for(int i = 0; i < cooks.length; i++)
					cooks[i].interrupt();
			}
